...
export default class Multiplayer {
    ...
    async initSupabase() {
        let createClient;
        try {
            // Try importing from local package first (standard for Vite)
            const module = await import('@supabase/supabase-js');
            createClient = module.createClient;
        } catch (e) {
            console.warn('Could not load @supabase/supabase-js from local modules, falling back to CDN', e);
    ...
    async cleanupOldRooms(maxAgeHours = 24) {
        if (!this.supabase && this.initPromise) {
            try {
                ...
                console.error('Failed to wait for Supabase init in cleanup:', e);
            }
        }
    ...
    async pingSupabase() {
        if (!this.supabase && this.initPromise) {
            try {
                await this.initPromise;
            } catch (e) { /* Ignore init errors in ping */ }
        }
    ...
    async createRoom(settings) {
        try {
            ...
            if (!this.supabase) {
                throw new Error('Multiplayer service unavailable. Please check your connection.');
            }
